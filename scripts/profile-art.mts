...
type Stat = readonly [label: string, value: number];
type Lang = readonly [name: string, size: number];
...
function isLangNode(value: unknown): value is LangNode {
  return isRec(value)
    && isStr(value.name)
    && (value.color === null || isStr(value.color));
}

function isLangEdge(value: unknown): value is LangEdge {
  return isRec(value)
    && isNum(value.size)
...
function parseGql(value: unknown): GqlRes {
  if (!isRec(value)) {
  ...
  if (errors !== undefined) {
    if (!Array.isArray(errors) || !errors.every((item): item is GqlError => isRec(item) && isStr(item.message))) {
      throw new Error("GitHub GraphQL errors have an invalid shape");
    }
  }

  const data = value.data;
  let parsedData: GqlRes["data"];
...
const excludedRepos = new Set<string>(["nsfwjs", "chatbot-ui", "carbonyl"]);
const langSkip = new Set<string>(["JavaScript", "HTML", "CSS"]);
...
const langs: Lang[] = [...langBytes.entries()]
  .sort((a, b) => b[1] - a[1])
  .slice(0, 7);
...
function shell(t: Theme, width: number, height: number, body: string): string {
...
function statsSvg(t: Theme): string {
...
function langsSvg(t: Theme): string {
  const total = langs.reduce((sum, [, size]) => sum + size, 0) || 1;
  const colours = [t.teal, t.lilac, t.cyan, "#388e3c", "#2f81f7", "#d29922", "#f778ba"] as const;
  let offset = 0;
...
function contribSvg(t: Theme): string {
  const width = 850;
  const height = 224;
  const size = 10;
  const gap = 3;
  ...
  const byDate = new Map(contributions.map((day) => [day.date, day] as const));
...
for (const [name, theme] of Object.entries(themes) as [keyof typeof themes, Theme][]) {
